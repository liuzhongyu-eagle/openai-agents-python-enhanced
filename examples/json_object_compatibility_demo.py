...
def demo_configuration():
    ...
    # 显示默认配置
    print("默认配置:")
    print(f"- 默认语言: {JsonObjectConfig.default_language}")
    print(f"- 包含示例: {JsonObjectConfig.default_include_examples}")
    print(f"- 验证模式: {JsonObjectConfig.default_validation_mode}")
    print(f"- 启用指令缓存: {JsonObjectConfig.enable_instruction_cache}")
    print(f"- 启用验证器缓存: {JsonObjectConfig.enable_validator_cache}")
    print(f"- 启用智能降级: {JsonObjectConfig.enable_smart_fallback}")
    ...
    # 创建新的输出模式以测试配置
    en_schema = JsonObjectOutputSchema(TaskItem)
    print("英文模式下的指令:")
    print(en_schema.get_system_prompt_injection()[:200] + "...")
    
    # 恢复原始配置
    JsonObjectConfig.default_language = original_language
    print(f"\n配置已恢复为: {JsonObjectConfig.default_language}")
